...
const Adminrent = () => {
    ...
    const handleAddProduct = async (type) => {
        ...
        switch (type) {
            case 'shirt':
                data = { ...newShirt, id: Date.now() }; // Generate a unique id using Date.now()
                break;
            case 'pant':
                data = { ...newPant, id: Date.now() };
                break;
            case 'trouser':
                data = { ...newTrouser, id: Date.now() };
                break;
            case 'tshirt':
                data = { ...newTshirt, id: Date.now() };
                break;
            case 'suite':
                data = { ...newSuite, id: Date.now() };
                break;
            default:
                return;
        }
        ...
        try {
            const response = await axios.post(`http://localhost:8080/user/${type}/add`, data);
            ...
            alert(`Error adding product: ${error.response ? error.response.data : error.message}`);
        }
    };
    const modalStyle = {
        position: 'absolute',
